...
const Header = () => {
  ...
  const menuItems = [
    { href: "/tourist-spots", label: "المخبوزات" },
    ...
    <header className="fixed top-0 w-full px-4 py-3 z-50 bg-transparent backdrop-blur-[2px] border-b border-gray-200 dark:border-gray-700 rounded-b-2xl shadow-sm">
      <div className="container mx-auto flex justify-between items-center lg:flex-row flex-row-reverse">
        ...
        <div className="flex lg:hidden items-center gap-x-4">
          <DarkModeToggle />
          <Link href="/shobeek-lobeek"> {/* Link to shobeek-lobeek page */}
            <SiGhostery className="w-6 h-6 text-seaBlue hover:text-sandyGold transition duration-300" />
          </Link>
          <button
        ...
        {/* الشعار */}
        <Link href="/" className="flex items-center lg:order-none order-2">
          <h1 dir="rtl" className="font-extrabold text-xl md:text-3xl text-amber-900 hover:text-sandyGold transition duration-300">
            خباز الخير
          </h1>
        </Link>
